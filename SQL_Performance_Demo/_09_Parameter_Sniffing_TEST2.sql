...
-- Most orders are 'Completed' (898,990)
-- Less orders are 'Pending' (100,000)
-- Few orders are 'Cancelled' (1,000)
-- Very few are 'Suspended' (10)
WITH Numbers AS (
    SELECT TOP (1000000) 
...
SELECT 
    RowID,
    (RowID % 5000) + 1,
    DATEADD(DAY, -(RowID % 730), GETDATE()),
    (RowID % 1000) + 100.00,
    CASE 
        WHEN RowID % 10 = 1 THEN 'Pending'
        WHEN RowID % 1000 = 5 THEN 'Cancelled'
        WHEN RowID % 100000 = 3 THEN 'Suspended'
        ELSE 'Completed'
    END,
    CASE (RowID % 4)
        WHEN 0 THEN 'North'
...
-- Scenario 1: First execution with 'Cancelled' (1,000 rows)
PRINT '=== First execution: Cancelled (1,000 rows) ===';
...
-- Scenario 2: Second execution with 'Completed' (898,990 rows) - USES SAME PLAN
PRINT '=== Second execution: Completed (898,990 rows) - SAME PLAN ===';

SET STATISTICS IO ON;
SET STATISTICS TIME ON;

EXEC dbo.GetOrdersByStatus @Status = 'Completed';
-- Uses cached plan from 'Cancelled', which is inefficient for 898,990 rows!
-- Index Seek + 898,990 Key Lookups = SLOW
GO
...
EXEC dbo.GetOrdersByStatus @Status = 'Cancelled';
-- Uses plan optimized for 898,990 rows on 1,000 rows = suboptimal
GO
...
EXEC dbo.GetOrdersByStatus_Recompile @Status = 'Cancelled';   -- Optimal plan for 1,000 rows
EXEC dbo.GetOrdersByStatus_Recompile @Status = 'Completed';   -- Optimal plan for 898,990 rows
...
BEGIN
    ...
    IF @Status IN ('Suspended', 'Cancelled', 'Pending')
    BEGIN
...
CROSS APPLY sys.dm_exec_sql_text(qs.sql_handle) st
CROSS APPLY sys.dm_exec_query_plan(qs.plan_handle) qp
WHERE st.text LIKE '%GetOrdersByStatus_PSP%';
